...
modelo = 'modelo_0'
path_ = 'modelo_3_sem_subgrupo'
########################################################################################################################
for periodo in lista_periodo:
    print(f'{periodo} - {modelo} - {missing} - rodada {contador} de {len(lista_periodo)}')
    contador += 1
    try:
        ...
        with open(f'resultados/{path_}/{periodo}_{modelo}_{missing}.txt', 'w') as f:
            f.write('---------------------------------------------------------------- \n')
        ...
        odds_ratio.to_csv(f'resultados/{path_}/{periodo}_{modelo}_{missing}.csv', decimal=',', sep=';',
                          index=False)
        ...
        plt.savefig(f'resultados/{path_}/fig1a_{periodo}_{modelo}_{missing}.png', format='png', dpi=300)
        plt.clf()
        # plt.show()
        #
        ...
        plt.legend(['Control', 'Treatment'])
        plt.savefig(f'resultados/{path_}/fig1b_{periodo}_{modelo}_{missing}.png', format='png', dpi=300)
        plt.clf()
        # plt.show()
        #
        # #spearm
        ...
        plt.savefig(f'resultados/{path_}/graf_corr_{periodo}_{modelo}_{missing}.png', format='png', dpi=300)
        plt.clf()
        # plt.show()
    except:
        lista_erro.append({'periodo': [periodo]})
        continue

lista_erro_ = pd.concat([pd.DataFrame(i) for i in lista_erro])
lista_erro_.to_csv(f'resultados/{path_}/erro_{modelo}_{missing}.csv', index=False)
