...
lista_escolaridade = ['Ensino_superior', 'Ensino_medio', 'Fundamental', 'Sem_escolaridade']
lista_erro = []
contador = 1
########################################################################################################################
# INPUT
...
for escolaridade in lista_escolaridade:
    ...
    for periodo in lista_periodo:
        ...
        try:
            ...
            plt.savefig(f'resultados/{path_}/graf_corr_{periodo}_{modelo}_{missing}_{escolaridade}.png', format='png', dpi=300)
            plt.clf()
            # plt.show()
        except:
...
lista_erro_.to_csv(f'resultados/{path_}/erro_{modelo}_{missing}_escolaridade.csv', index=False)
...
for escolaridade in lista_escolaridade:
    ...
    for periodo in lista_periodo:
        ...
        try:
            ...
            odds_ratio.to_csv(f'resultados/{path_}/{periodo}_{modelo}_{missing}_{escolaridade}.csv', decimal=',',
                              sep=';',
                              index=False)
            ...
            fig = sns.kdeplot(df_mod.query("ANO==1")["PROPENSITY_SCORE"], bw_adjust=.7, shade=False, color="b")
            plt.legend(['Control', 'Treatment'])
            plt.savefig(f'resultados/{path_}/fig1a_{periodo}_{modelo}_{missing}_{escolaridade}.png', format='png',
                        dpi=300)
            plt.clf()
            # plt.show()
            #
            ...
            fig = sns.kdeplot(psw_base_balanced.query("ANO==1")["PROPENSITY_SCORE"], bw_adjust=0.7, shade=False,
                              color="b")
            plt.legend(['Control', 'Treatment'])
            plt.savefig(f'resultados/{path_}/fig1b_{periodo}_{modelo}_{missing}_{escolaridade}.png', format='png',
                        dpi=300)
            plt.clf()
            # plt.show()
            #
            ...
            heatmap = sns.heatmap(psw_base_balanced[var_corr].corr(method='spearman'), mask=mask, vmin=-1, vmax=1,
                                  ...
                                  cmap='BrBG')
            heatmap.set_title('Matrix', fontdict={'fontsize': 18}, pad=16)
            plt.savefig(f'resultados/{path_}/graf_corr_{periodo}_{modelo}_{missing}_{escolaridade}.png',
                        format='png',
                        dpi=300)
            plt.clf()
            # plt.show()
