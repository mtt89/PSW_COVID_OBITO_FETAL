...
for regiao in lista_regiao:
    ...
    for periodo in lista_periodo:
        print(f'{periodo} - {regiao} - rodada {contador} de {len(lista_periodo)*len(lista_regiao)}')
        contador += 1
        try:
            ...
            with open(f'resultados/modelo_3_subgrupo_regiao/{periodo}_modelo0_{missing}_OBITO.txt', 'w') as f:
                f.write('---------------------------------------------------------------- \n')
            ...
            odds_ratio.to_csv(f'resultados/modelo_3_subgrupo_regiao/{periodo}_modelo0_{missing}_OBITO.csv', decimal=',', sep=';',
                              index=False)
            ...
            plt.savefig(f'resultados/modelo_3_subgrupo_regiao/fig1a_{periodo}_modelo0_{missing}_OBITO.png', format='png', dpi=300)
            plt.clf()
            # plt.show()
            #
            ...
            plt.legend(['Control', 'Treatment'])
            plt.savefig(f'resultados/modelo_3_subgrupo_regiao/fig1b_{periodo}_modelo0_{missing}_OBITO.png', format='png', dpi=300)
            plt.clf()
            # plt.show()
            #
            # #spearm
            ...
            plt.savefig(f'resultados/modelo_3_subgrupo_regiao/graf_corr_{periodo}_modelo0_{missing}_OBITO.png', format='png',
                        dpi=300)
            plt.clf()
...
lista_erro_.to_csv('resultados/modelo_3_subgrupo_regiao/erro_modelo0_regiao_com_input.csv', index=False)
